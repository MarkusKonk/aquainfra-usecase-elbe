...
class ProcessDasymetricRefinementProcessor(BaseProcessor):
    ...
    def __repr__(self):
        return f'<ProcessDasymetricRefinementProcessor> {self.name}'
    ...
    def execute(self, data, outputs=None):
        ...
        in_inputFile2_rds = data.get('inputFile2_rds')
        in_inputFile3_gpkg = data.get('inputFile3_gpkg')
        in_inputFile4_gpkg = data.get('inputFile4_gpkg')
        ...
        if in_inputFile2_rds is None:
            raise ProcessorExecuteError('Missing parameter "inputFile2_rds". Please provide a inputFile2_rds.')
        ...
        if in_inputFile4_gpkg is None:
            raise ProcessorExecuteError('Missing parameter "inputFile4_gpkg". Please provide a inputFile4_gpkg.')

        # Where to store output data
        downloadfilename = 'ancillary_data-%s.gpkg' % self.my_job_id
        ...
        if not returncode == 0:
            ...
            response_object = {
                "outputs": {
                    "ancillary_data": {
                        "title": self.metadata['outputs']['ancillary_data']['title'],
                        "description": self.metadata['outputs']['ancillary_data']['description'],
                        "href": f'{downloadlink}'
                    }
                }
            }
...
def run_docker_container(
    ...
    script = 'process_dasymetric_refinement.R'
